...
// Import the task templates data
import { taskTemplates, getAllTaskTemplates } from './shared/taskTemplates.js';
...
async function main() {
  ...
  try {
    ...
    for (const tier1 of tier1Categories) {
      let tier1Category = existingCategories.find(cat => 
      ...
      for (const tier2 of tier2Categories[tier1]) {
        let tier2Category = existingCategories.find(cat => 
          cat.type === 'tier2' && 
