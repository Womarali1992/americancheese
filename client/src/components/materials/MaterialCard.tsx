...
export function MaterialCard({ material, onEdit, onDelete }: MaterialCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Modern gradient header with material name and icon */}
      <div className="bg-gradient-to-r from-orange-600 to-amber-500 pt-5 pb-8 px-5 text-white relative overflow-hidden">
        {/* Background pattern */}
      ...
      {/* Elevated quantity and cost summary */}
      <div className="flex justify-between mx-5 -mt-4 relative z-20">
        <div className="bg-white shadow-md rounded-lg px-3 py-2 flex-1 flex items-center justify-between border border-orange-100">
          <div>
            <p className="text-[10px] text-orange-600 font-semibold uppercase">Quantity</p>
            <p className="text-sm font-bold">{material.quantity} <span className="text-xs font-normal text-orange-700">{material.unit || 'units'}</span></p>
          </div>
          <div className="border-l border-orange-100 pl-3">
            <p className="text-[10px] text-orange-600 font-semibold uppercase">Total Cost</p>
            <p className="text-sm font-bold text-orange-700">{totalCost ? formatCurrency(totalCost) : '$0.00'}</p>
          </div>
        </div>
      </div>
      ...
      <CardContent className="p-5 pt-7">
        {/* Classification tags with updated design */}
        {(material.tier2Category || material.section || material.subsection) && (
          <div className="mb-4 flex flex-wrap gap-1.5">
            {material.tier2Category && (
              <span className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-orange-50 text-orange-800 border border-orange-100">
                {material.tier2Category}
              </span>
            )}
            {material.section && (
        ...
        <div className="grid grid-cols-2 gap-4 mb-3">
          <div className="bg-orange-100/40 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
        ...
        <div className="flex items-center justify-between bg-orange-100/40 px-3 py-2 rounded-lg mb-4">
